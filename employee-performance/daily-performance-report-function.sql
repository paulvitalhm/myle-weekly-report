...
-- This calculates performance index per day (team performance) instead of per worker

-- Create function for daily performance report
...
-- Test the daily performance report
SELECT * FROM get_daily_performance_index('91a20127-5cda-4220-a405-791640417f5d')
ORDER BY day_date;

-- Verification query to manually check calculations
SELECT 
    wd.work_date,
    SUM(a.cost) as total_revenue,
    SUM(wd.hours_worked) as total_hours,
